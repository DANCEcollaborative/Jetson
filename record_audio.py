...
def main(args):
    ...
    RESPEAKER_CHANNELS = (
        args.num_channels
    )  # change base on firmware, 1_channel_firmware.bin as 1 or 6_channels_firmware.bin as 6
    ...
    socket = create_socket(ip_address=f"tcp://*:{audio_port}")  # channel to send audio
    socket2 = create_socket(ip_address=f"tcp://*:{doa_port}")  # channel to send DOA
    socket3 = create_socket(ip_address=f"tcp://*:{vad_port}")  # channel to send VAD
    ...
    audio_output_files = []
    dirs_file = open(
        os.path.join(audio_dir, "doa.txt"), "w+"
    )  # txt file t store every polled DOA value
    ...
    for i in range(RESPEAKER_CHANNELS):
        wf = wave.open(
            os.path.join(audio_dir, WAVE_OUTPUT_FILE_PREFIX + f"{i}.wav"), "wb"
        )
        wf.setnchannels(1)
    ...
    try:
        while True:
            ...
            )  # if TRUE, the audio recording could stop automatically if the audio data exceeds system buffer capacity
            ...
            for i in range(RESPEAKER_CHANNELS):
                channel_audio = np.fromstring(data, dtype=np.int16)[
                    i::RESPEAKER_CHANNELS
                ].tostring()
                ...
                if i == 0:
                    ...
                    print(
                        f"Channel 0 audio sent at {originatingTime}", len(channel_audio)
                    )
            ...
            if Mic_tuning is not None:
                ...
                print(
                    f"DOA {dirn} - VAD {vad} | sent at {originatingTime2}",
                    originatingTime == originatingTime2,
                )
        ...
        dirs_file.close()
        print(
            f"Closed all open doa file and audio file pointers. Number of chunks recorded = {frames_saved}. Sample rate = {RESPEAKER_RATE}. Num channels saved = {RESPEAKER_CHANNELS}"
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default=6,
        type=int,
        help="Number of channels being recorded by microphone",
    )
    parser.add_argument(
